...
function getColumnDecoratedFields<T extends new (...args: any[]) => any>(
  ...
  for (const property of properties) {
    ...
    if (hasColumn) {
      ...
      // 获取ApiProperty装饰器的元数据
      const apiPropertyMetadata = Reflect.getMetadata('swagger/apiProperties', target.prototype, property);
      
      decoratedFields.push({ 
        propertyName: property, 
        type: type,
        description: apiPropertyMetadata?.description,
        example: apiPropertyMetadata?.example
      });
    }
  }
