...
import {requestArtColumnDel, requestArtColumnList} from "@/network/article/articleColumn";
import {resInterface} from "@/commonNetwork/index";
import {handleGetCurInstance} from "@/utils/utils";
import {pageConfig, tableConfig} from "@/views/article/articleColumn/config/tableConfig";
import {formInit,formConfig} from "@/views/article/articleColumn/config/formConfig";
...
export const useFunc = () =>{
    ...
    let artColumnEditDialogRef = ref<InstanceType<typeof G_ArticleColumnEditDialog>>()
    let artColumnAddDialogRef = ref<InstanceType<typeof G_ArticleColumnAddDialog>>()
    ...
    const handleList = ()=>{
        ...
        return requestArtColumnList(form).then((res:resInterface)=>{
            ...
            for(let i in list){
                tableConfig.tableData.push({
                    ...list[i],
                    isShow:list[i]['isShow']==1?true:false,
                    block_comment:list[i]['block_comment']==1?true:false,
                })
            }
            return true;
        }).catch((err: any)=>{
    ...
    const handleDelete = (row?: any)=>{
        ...
        model.handleMsgBox(i18n.t("msg.areYouSureToDelete")).then(()=>{
            loading.value=true
            return requestArtColumnDel({id:id.toString()}).then(res=>{
    ...
    //编辑
    const handleEdit = (row: any,artColumnEditDialogRef:any)=>{
        artColumnEditDialogRef.handleOpenDialog(row)
    }
    //添加
    const handleAdd = (artColumnAddDialogRef:any) => {
        artColumnAddDialogRef.handleOpenDialog()
    }
    ...
    return {
        ...
        tableListRef,artColumnEditDialogRef,artColumnAddDialogRef,tableConfig,settingStore,multiple,dialogShow,dialogTitle,key,formSearchModel,formConfig,pageConfig,loading
    }
}