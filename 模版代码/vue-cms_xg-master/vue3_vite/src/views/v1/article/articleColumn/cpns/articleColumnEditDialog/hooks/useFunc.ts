...
import {requestArtColumnUpdate} from "@/network/article/articleColumn";
import {resInterface} from "@/commonNetwork/index";
import {formInit} from "@/views/article/articleColumn/cpns/articleColumnEditDialog/config/formConfig";
import {ref} from "vue";
...
export const useFunc = () =>{
    ...
    const handleEditSubmit = async (formRef:InstanceType<typeof FormList>,emit:any)=>{
        ...
        requestArtColumnUpdate(form).then((res:resInterface)=>{
    ...
    const handleOpenDialog = (val:any)=> {
        dialogTitle.value = "编辑文章栏目"
        key.value = Math.random()
        dialogShow.value = true
        formModel.value = {...val}
    }
    return {
        handleOpenDialog,handleResetForm,handleEditSubmit,handleCancel,formRef,dialogShow,loading,dialogTitle,key,formModel
    }
}