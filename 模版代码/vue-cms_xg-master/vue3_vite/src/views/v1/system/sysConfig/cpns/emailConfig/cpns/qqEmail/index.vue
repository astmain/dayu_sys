...
-->
<template>
  <Loading :isLoading="loading" style="width:100%">
    <FormList v-model="formModel" ref="formRef" :formConfig="formConfig">
      <template #formCustomBtn>
        <el-button type="primary" v-if="handleCheckPower(['system:sysConfig:emailConfig:qqEmail'])" :loading="loading" @click.native.prevent="handleSubmit(formRef)">{{$t("btn.update")}}</el-button>
...
let {loading, formModel,formRef, handleSubmit, handleResetForm} = useFunc()
