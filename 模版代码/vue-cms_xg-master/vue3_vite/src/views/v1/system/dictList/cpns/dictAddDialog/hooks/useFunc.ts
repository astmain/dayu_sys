...
import {requestDictAdd} from "@/network/system/dictList/index";
...
export const useFunc = () =>{
    ...
    const handleAddSubmit = async (formRef:InstanceType<typeof FormList>,emit:any)=>{
        ...
        requestDictAdd(form).then((res:resInterface)=>{
    ...
    //初始化弹出框
    const handleOpenDialog = async (row?:any):Promise<boolean> => {
        await handleInit()
    ...
    const handleDealFormData = (dicts)=>{
        for(let i in formConfig.formItems){
            if(formConfig.formItems[i]["prop"] == "statue"){
