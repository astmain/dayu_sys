...
import {requestDictDataAdd} from "@/network/system/dictList/data";
...
export const useFunc = () =>{
    ...
    const handleAddSubmit = async (formRef:InstanceType<typeof FormList>,emit:any)=>{
        ...
        requestDictDataAdd(form).then((res:resInterface)=>{
    ...
    //初始化弹出框
    const handleOpenDialog = async (curDictId:any):Promise<boolean> => {
        console.log(curDictId,"curDictIdcurDictId");
        await handleInit(curDictId)
    ...
    //初始化数据
    const handleInit = async (curDictId:any)=>{
        await handleDict(dict);
        await handleGetDetail(curDictId)
    ...
    const handleGetDetail = (id:number|string)=>{
        requestDictDetail({id}).then((res:any) => {
