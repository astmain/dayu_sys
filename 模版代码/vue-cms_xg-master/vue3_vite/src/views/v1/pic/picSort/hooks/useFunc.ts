...
import {formInit,formConfig} from "@/views/pic/picSort/config/formConfig";
import {requestImgSortDel, requestImgSortList} from "@/network/pic/picSort";
import {pageConfig,tableConfig} from "@/views/pic/picSort/config/tableConfig";
...
export const useFunc = () =>{
    ...
    let picSortEditDialogRef = ref<InstanceType<typeof G_PicSortEditDialog>>()
    let picSortAddDialogRef = ref<InstanceType<typeof G_PicSortAddDialog>>()
    ...
    const handleList = ()=>{
        ...
        return requestImgSortList(form).then((res:resInterface)=>{
    ...
    const handleDelete = (row?: any)=>{
        ...
        model.handleMsgBox(i18n.t("msg.areYouSureToDelete")).then(()=>{
            loading.value=true
            return requestImgSortDel({id:id.toString()}).then(res=>{
    ...
    //编辑
    const handleEdit = (row: any,picSortEditDialogRef:any)=>{
        picSortEditDialogRef.handleOpenDialog(row)
    }
    //添加
    const handleAdd = (picSortAddDialogRef:any) => {
        picSortAddDialogRef.handleOpenDialog()
    }
    ...
    const handleToImgListBySortId =(id)=>{
        router.push({ path: '/pic/picList-index/'+id })
    }
    onMounted(()=>{
        loading.value = true;
    ...
    return {
        handleToImgListBySortId,handleTableSelectionChange,handleList,handleSearch,handleReset,handleSizeChange,handleCurrentChange,handleCloseDialog,handleDelete,handleEdit,handleAdd,
        appStore,tableListRef,picSortEditDialogRef,picSortAddDialogRef,picListDialogRef,tableConfig,settingStore,multiple,dialogShow,dialogTitle,key,formSearchModel,formConfig,pageConfig,loading
    }
}